...
import { formatarPrecisao, validarProposta } from "../utils/venda.js";

const VERDE = BRAND.primary;
const VERDE_CLARO = BRAND.primarySoft;
const CINZA = "A6A6A6";
const BRANCO = "FFFFFF";
const PRETO = "111111";
const FONTE = "Calibri";
          ...
          new TableRow({
            children: [
              new TableCell({
                ...
                children: [
                  new Paragraph({
                    children: [run(BRAND.companyNameUpper, { bold: true, size: 20, color: CINZA })],
                    spacing: { after: 0 },
                  }),
                  new Paragraph({
                    children: [run(BRAND.address, { size: 20, color: CINZA })],
                    spacing: { after: 0 },
                  }),
                  new Paragraph({
                    children: [run(BRAND.cityState, { size: 20, color: CINZA })],
                    spacing: { after: 0 },
                  }),
                  new Paragraph({
                    children: [run(`Telefone: ${BRAND.phone}`, { size: 20, color: CINZA })],
                    spacing: { after: 0 },
                  }),
                ],
              }),
...
const criarTabelaValores = (grupos, totalGeral, descontoNegociacao = 0) => {
  validarProposta(grupos, totalGeral, descontoNegociacao);
      ...
      linhas.push(
        new TableRow({
          ...
          children: [
            ...
            celula(formatarPrecisao(item.quantidade), { alignment: AlignmentType.RIGHT }),
            celula(`R$ ${formatarPrecisao(item.unitario)}`, { alignment: AlignmentType.RIGHT }),
...
export const criarPropostaAlphaDocxBlob = async (dados, opcoes = {}) => {
  const respostaLogo = opcoes.logoData ? null : await fetch(jamLogo);
  if (respostaLogo && !respostaLogo.ok) throw new Error("Não foi possível carregar a logo da JAM.");
  ...
  const abertura = [
    ...
    paragrafo(`Lagoa Santa ${dataHoje}`, { alignment: AlignmentType.RIGHT, after: 240 }),
  ...
  const corpo = [
    tituloSecao(`Responsabilidade da ${BRAND.companyNameUpper}:`, { before: 200 }),
  ...
  fechamento.push(
    new Paragraph({
      children: [run(BRAND.companyNameUpper, { bold: true })],
      alignment: AlignmentType.CENTER,
...
export const gerarPropostaAlphaDocx = async (dados) => {
  ...
  baixarBlob(blob, `${nomeArquivoSeguro(dados.nomeProjeto)}_Proposta_JAM.docx`);
};

...
import { FONTES_PADRAO, TIPOS, seedCpus } from "./data/defaultData";
import {
...
import {
  ...
  insumoEhMaterial,
  itemVendaResumo, montarItensProposta, calcularPrecoVendaProjeto,
  valorVendaUnitarioInsumo, validarProposta, arredondarMoeda, somarMoeda, formatarPrecisao, ratearMoeda,
} from "./utils/venda.js";
import {
...
import jamLogo from "./assets/jam-terraplanagem-logo.png";
import { BRAND, BRAND_FOOTER_HTML } from "./config/brand";
...
const chaveClienteCompartilhado = (cliente = {}) => {
  ...
  if (nome && telefone) return `nome:${nome}|tel:${telefone}`;
...
const exportarPropostaXlsx = ({ projeto, cliente, etapas, bdiCalc, cpus, catalogMap, modelo }) => {
  ...
  validarProposta(grupos, bdiCalc.valorVenda, bdiCalc.descontoNegociacao || 0, bdiCalc.problemas);
  ...
  const sufixo = modelo === "collem" ? "Anexo_I_COLLEM" : "Proposta_JAM";
...
const gerarPropostaPdf = ({ projeto, cliente, etapas, bdiCalc, cpus, catalogMap, numeroPropostaAutomatico }) => {
  ...
  const descricaoRegimeMateriais = materialPorContaCliente(cliente)
    ...
    : materialFaturamentoDireto(cliente) || bdiCalc.faturamentoDireto
      ...
      : `Materiais inclusos no fornecimento da ${BRAND.companyNameUpper} conforme composição do orçamento.`;
    ...
    .map((grupo) => `
      ...
      ${grupo.itens.map((item) => `
        <tr>
          ...
          <td>${formatarPrecisao(item.quantidade)}</td>
          <td>R$ ${formatarPrecisao(item.unitario)}</td>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    :root { --alpha: #F59E0B; --alpha-dark: #111111; --alpha-soft: #FFF3D6; --line: #F6C453; --alpha-gray: #737373; }
...
<body>
  <section class="page">
    <header>
      <img class="logo" src="${escapeHtml(jamLogo)}" alt="JAM Terraplanagem" />
    ...
    <p class="data">Lagoa Santa ${escapeHtml(dataHoje)}</p>
    ...
    <div class="footer">
      <div class="endereco">${BRAND_FOOTER_HTML}</div>
  ...
  <section class="page">
    ...
    <h2>Responsabilidade da ${BRAND.companyNameUpper}:</h2>
  ...
  <section class="page">
    ...
    <div class="assinatura">${BRAND.companyNameUpper}</div>
    <div class="footer">
      <div class="endereco">${BRAND_FOOTER_HTML}</div>
...
const calcularComparativosAlternativas = (etapas, bdi, cpus, catalogMap, cliente = {}) =>
  (etapas || []).flatMap((etapa) =>
    gruposAlternativasDaEtapa(etapa).map((grupo) => ({
      etapaId: etapa.id,
      etapaNome: etapa.nome,
...
export default function App() {
  ...
  const [clientesDirty, setClientesDirty] = useState(false);
  ...
  const [novoOrcamentoClienteId, setNovoOrcamentoClienteId] = useState("");
  const [novoOrcamentoNome, setNovoOrcamentoNome] = useState("");
  ...
  const setClientes = (nextClientes) => {
    setClientesDirty(true);
    setClientesState(nextClientes);
  };
  ...
  const aplicarDadosCarregados = (data) => {
    if (data.empty) {
      ...
      setProjetos([]);
      setClientesState([]);
      setClientesDirty(false);
      projectHashesRef.current = {};
      legacyPrecosRef.current = [];
      setProjetoAtivoId("");
      setTab("clientes");
      setStatus("Cadastre o primeiro cliente para começar um orçamento.");
    ...
    setClientesState(cadastroCompartilhado.clientes);
    setClientesDirty(
      JSON.stringify(data.clientes || []) !== JSON.stringify(cadastroCompartilhado.clientes)
    );
  ...
  const carregarDados = async ({ usarDrive = true } = {}) => {
    ...
    try {
      ...
      if (usarDrive && driveData) {
        ...
        const data = await loadLocalSnapshot();
        aplicarDadosCarregados(data || { empty: true });
      }
    } catch (e) {
      ...
      try {
        ...
        if (local) {
          ...
          setStatus("Google Drive indisponível. Dados carregados do backup local deste navegador.");
        } else {
  ...
  const salvarClientesCompartilhados = async (proximosClientes) => {
    const clientesAtualizados = [...(proximosClientes || [])].sort((a, b) =>
      String(a.nome || "").localeCompare(String(b.nome || ""), "pt-BR")
    );
    setBusy(true);
    setStatus("Salvando cadastro compartilhado de clientes...");
    try {
      await executarSalvamentoAteConseguir("o cadastro de clientes", async () => {
        const dados = dadosAtuaisRef.current;
        const clientesPorId = new Map(clientesAtualizados.map((cliente) => [cliente.id, cliente]));
        const projetosAtualizados = dados.projetos.map((projeto) => {
          const cadastroAtual = clienteDoProjeto(projeto);
          const compartilhado = clientesPorId.get(cadastroAtual.clienteId);
          if (!compartilhado) return projeto;
          const clienteCadastro = {
            ...cadastroAtual,
            ...dadosClienteCompartilhado(compartilhado),
            clienteId: compartilhado.id,
          };
          return {
            ...projeto,
      ...
      setClientesDirty(false);
      setDriveConnected(true);
      setStatus("Cadastro de clientes salvo no Google Drive.");
      return true;
    } finally {
      setBusy(false);
  ...
  const modeloPropostaAtivo = clienteAtivo.modeloProposta === "collem" ? "alpha" : (clienteAtivo.modeloProposta || "alpha");
  ...
  const executarMudancaAba = (proximaTab, validarCadastroProjeto = false) => {
    if (validarCadastroProjeto && proximaTab !== "cliente" && !cadastroClienteOk) {
      setTab("cliente");
      setStatus("Selecione um cliente cadastrado e informe o Local da obra para continuar.");
      setTimeout(() => setStatus(""), 5000);
      return;
    }
    setTab(proximaTab);
...
// NOVO: Função para varrer e consolidar o quantitativo de materiais
  ...
  const bdiCalc = useMemo(() => {
    const calculo = calcularPrecoVendaProjeto(etapas, bdi, cpus, catalogMap, clienteAtivo);
    const descontoNegociacao = Math.min(
      calculo.valorVenda,
      arredondarMoeda(descontoNegociacaoProjeto(projetoAtivo))
    );
    const valorVenda = aplicarDescontoNegociacao(
      calculo.valorVenda,
      descontoNegociacao
    );
    const totalDiValor = valorVenda - calculo.custoDiretoContratado;
    return {
      ...calculo,
      ...
      totalDiValor,
      totalDiRate:
        calculo.custoDiretoContratado > 0 ? totalDiValor / calculo.custoDiretoContratado : 0,
    };
  }, [bdi, etapas, cpus, catalogMap, projetoAtivo, clienteAtivo]);

  const validarExportacao = () => {
    try {
      validarProposta(bdiCalc.grupos, bdiCalc.valorVenda, bdiCalc.descontoNegociacao, bdiCalc.problemas);
      if (bdiCalc.avisos.length) return window.confirm(`${bdiCalc.avisos.join("\n")}\n\nConfirma que revisou esses avisos e deseja gerar a proposta?`);
      return true;
    } catch (erro) {
      alert(erro.message);
      return false;
  ...
  const valorVendaItem = (item, key) => {
    const { quantidade, unitario, total } = itemVendaResumo(item, bdiCalc, cpus, catalogMap);
    if (key === "estrutura") return item.servico || item.descricao || item.codigo || "";
    if (key === "unidade") return item.unidade || "";
    if (key === "quantidade") return quantidade;
  ...
  const valorVendaInsumo = (insumo, quantidadeItem, key) => {
    const unitario = valorVendaUnitarioInsumo(insumo, bdiCalc, cpus, catalogMap);
    const quantidade = num(insumo.coeficiente) * quantidadeItem;
    if (key === "estrutura") return insumo.descricao || insumo.codigo || "";
    if (key === "unidade") return insumo.unidade || "";
    if (key === "quantidade") return quantidade;
  ...
  const comparativosAlternativas = useMemo(
    () =>
      calcularComparativosAlternativas(etapas, bdi, cpus, catalogMap, clienteAtivo).map(
        (grupo) => ({
          ...grupo,
          opcoes: grupo.opcoes.map((opcao) => ({
            ...opcao,
    ...
    [etapas, bdi, cpus, catalogMap, bdiCalc.descontoNegociacao, clienteAtivo]
  );
  ...
  const abrirNovoOrcamento = () => {
    if (clientes.length === 0) {
      setTab("clientes");
      setStatus("Cadastre e salve um cliente antes de criar o primeiro orçamento.");
      setTimeout(() => setStatus(""), 6000);
      return;
    }
    setNovoOrcamentoClienteId("");
    setNovoOrcamentoNome(`Novo Orçamento - ${projetos.length + 1}`);
    setNovoOrcamentoAberto(true);
  };

  const criarNovoOrcamento = () => {
    const clienteSelecionado = clientes.find((cliente) => cliente.id === novoOrcamentoClienteId);
    ...
    setProjetos((prev) => {
      ...
      return [
        ...
        {
          id: pId,
          nome: novoOrcamentoNome.trim() || `Novo Orçamento - ${prev.length + 1}`,
          numeracaoPropostaVersao: VERSAO_NUMERACAO_PROPOSTAS,
          ...
          clienteCadastro: {
            ...CLIENTE_PADRAO,
            ...dadosClienteCompartilhado(clienteSelecionado),
    ...
    setTab("cliente");
    setStatus(`Orçamento criado para ${clienteSelecionado.nome}. Informe o local da obra para continuar.`);
    setTimeout(() => setStatus(""), 6000);
  };
    ...
    <div className="min-h-screen bg-stone-50 text-stone-900 flex flex-col">
      {!driveConnected && (
        ...
        >
          <div className="w-full max-w-sm bg-white border border-stone-200 rounded-lg shadow-2xl p-6 text-center">
            <img
              src={jamLogo}
              alt="JAM Terraplanagem"
              className="w-40 h-24 object-contain rounded-sm mx-auto mb-4"
            />
      ...
      {novoOrcamentoAberto && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="novo-orcamento-title"
        >
          <div className="w-full max-w-lg bg-white border border-stone-200 rounded-lg shadow-2xl p-6">
            <div>
              <h2 id="novo-orcamento-title" className="text-base font-semibold text-stone-900 flex items-center gap-2">
                <FolderKanban size={18} /> Criar novo orçamento
              </h2>
              <p className="mt-1.5 text-sm text-stone-500">
                Selecione um cliente previamente cadastrado. Os dados permanentes serão vinculados automaticamente.
              </p>
            </div>
            ...
            <div className="mt-5 space-y-4">
              <CampoCliente
                ...
                value={novoOrcamentoNome}
                onChange={setNovoOrcamentoNome}
              ...
              <div>
                <label className="text-xs font-medium text-stone-600 mb-1 flex items-center gap-1.5">
                  <User size={14} /> Cliente cadastrado <span className="text-amber-600">*</span>
                </label>
                <select
                  value={novoOrcamentoClienteId}
                  onChange={(e) => setNovoOrcamentoClienteId(e.target.value)}
                  autoFocus
                  ...
                  {[...clientes]
                    ...
                    .map((cliente) => (
                      <option key={cliente.id} value={cliente.id}>
                        {cliente.nome || "Cliente sem nome"}{cliente.documento ? ` — ${cliente.documento}` : ""}
                      </option>
                    ))}
                </select>
              </div>
            ...
            <div className="mt-6 flex flex-col-reverse sm:flex-row sm:justify-between gap-2">
              ...
              <div className="flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setNovoOrcamentoAberto(false)}
                  className="px-4 py-2 text-sm border border-stone-300 rounded-md bg-white text-stone-700 hover:bg-stone-50"
                >
                  Cancelar
                </button>
                <button
                  ...
                  disabled={!novoOrcamentoClienteId || !novoOrcamentoNome.trim()}
                  className="px-4 py-2 text-sm font-medium bg-stone-900 text-white rounded-md hover:bg-stone-800 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Criar orçamento
                </button>
              </div>
      ...
      <div className="max-w-[1500px] w-full mx-auto px-4 py-6 lg:flex lg:items-start lg:gap-5 flex-1">
        <aside className="lg:sticky lg:top-4 lg:w-64 lg:shrink-0 mb-5 lg:mb-0">
          <div className="bg-white border border-stone-200 shadow-sm rounded-lg overflow-hidden">
            ...
            <nav className="p-2 space-y-1 max-lg:flex max-lg:overflow-x-auto max-lg:space-y-0 max-lg:gap-1">
              ...
              <SideTabBtn active={tab === "clientes"} onClick={() => solicitarMudancaAba("clientes")} icon={<User size={15} />}>
                Clientes ({clientes.length})
                {clientesDirty && (
                  <span className="ml-auto w-1.5 h-1.5 rounded-full bg-amber-500 inline-block" title="Alterações não salvas" />
                )}
              </SideTabBtn>
        ...
        <header className="flex items-center justify-between mb-6 gap-4">
          <div className="min-w-0 flex items-center gap-3">
            <img
              src={jamLogo}
              alt="JAM Terraplanagem"
              className="w-24 h-14 rounded-sm object-contain shrink-0"
            />
            <div className="min-w-0">
        ...
        {tab === "clientes" && (
          <ClientesTab
            clientes={clientes}
            projetos={projetos}
            onSalvar={salvarClientesCompartilhados}
            saving={busy}
            dirty={clientesDirty}
          />
        )}
        {tab === "cliente" && projetoAtivo && (
          <CadastroCliente
            ...
            onGerenciarClientes={() => setTab("clientes")}
          />
        )}
...
{tab === "bdi" && projetoAtivo && (
          <BdiTab bdi={bdi} setBdi={setBdi} bdiCalc={bdiCalc} grandTotal={bdiCalc.custoDiretoContratado} />
        )}
                          ...
                          return (
                            <div key={itemId} className="border-b border-stone-100">
                              ...
                              >
                                ...
                                <span className="col-span-1.5 text-right font-mono">{formatarPrecisao(qtdItem)}</span>
        ...
        {tab === "precovenda" && projetoAtivo && (
          <div className="bg-white border border-stone-200 shadow-sm rounded-lg overflow-hidden p-5 space-y-4">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 border border-stone-200 bg-stone-50 px-4 py-3 rounded-lg">
              <div>
                ...
                <p className="text-[11px] text-stone-500 mt-0.5">
                  Os documentos comerciais usam a identidade da JAM Terraplanagem.
                </p>
              </div>
              <div className="inline-flex w-full sm:w-auto border border-amber-300 rounded-md overflow-hidden bg-white" aria-label="Modelo da proposta JAM">
                <button
                  type="button"
                  ...
                  className="flex-1 sm:flex-none px-4 py-2 text-xs font-semibold bg-amber-500 text-stone-950"
                >
                  JAM TERRAPLANAGEM
                </button>
              </div>
              ...
              <div className="flex gap-2">
                ...
                {modeloPropostaAtivo && (
                  <button
                    ...
                    className={`px-2 py-1 text-[11px] font-medium border text-stone-950 rounded flex items-center gap-1 ${modeloPropostaAtivo === "collem" ? "border-amber-500 bg-amber-500 hover:bg-amber-400" : "border-amber-500 bg-amber-500 hover:bg-amber-400"}`}
                  >
            ...
            <div id="area-planilha-venda" className="border border-stone-200 rounded-lg overflow-hidden bg-white">
              <div className="px-4 py-2 text-xs text-stone-600 bg-stone-50 border-b border-stone-200">
                Valores exibidos com duas casas decimais. Os cálculos preservam a precisão original; cada CPU é arredondada a centavos e etapas e total somam esses valores.
                {" Campos numéricos em branco são considerados zero."}
                {materialPorContaCliente(clienteAtivo) && " Materiais por conta do cliente são excluídos, preservando mão de obra, equipamentos e outros serviços dentro das subcomposições."}
              </div>
              {bdiCalc.problemas.length > 0 && (
                <div role="alert" className="p-3 text-xs bg-red-50 text-red-800 border-b border-red-200">
                  <strong>Proposta bloqueada: revise os dados antes de exportar.</strong>
                  <ul className="list-disc pl-5 mt-1">{bdiCalc.problemas.slice(0, 8).map((problema) => <li key={problema}>{problema}</li>)}</ul>
                </div>
              )}
              {bdiCalc.avisos.length > 0 && (
                <div className="p-3 text-xs bg-amber-50 text-amber-900 border-b border-amber-200">
                  <strong>Avisos para conferência:</strong>
                  <ul className="list-disc pl-5 mt-1">{bdiCalc.avisos.map((aviso) => <li key={aviso}>{aviso}</li>)}</ul>
                </div>
              )}
              <div className="grid grid-cols-12 gap-2 px-4 py-2.5 bg-stone-100 border-b border-stone-200 text-stone-500 font-semibold text-[11px] uppercase tracking-wider">
              ...
              <div className="divide-y divide-stone-200 max-h-[600px] overflow-y-auto">
                {etapas.length === 0 ? (
                  ...
                  etapasVendaOrdenadas.map((etapa, idxEtapa) => {
                    ...
                    const totalEtapaComBdi = somarMoeda(itensAtivosDaEtapa(etapa).map(
                      (item) => itemVendaResumo(item, bdiCalc, cpus, catalogMap).total
                    ));
                    ...
                    return (
                      <div key={etapaId} className="bg-stone-50/30">
                        ...
                        {isEtapaAberta && ordenarLista(itensAtivosDaEtapa(etapa), ordenacaoPlanilhaVenda, valorVendaItem).map((item, idxItem) => {
                          ...
                          const resumoVenda = itemVendaResumo(item, bdiCalc, cpus, catalogMap);
                          const totalCpuComBdi = resumoVenda.unitario;
                          ...
                          return (
                            <div key={itemId} className="border-b border-stone-100">
                              ...
                              >
                                ...
                                <span className="col-span-1 text-center font-mono text-stone-400">{item.unidade}</span>
                                <span className="col-span-1.5 text-right font-mono">{formatarPrecisao(qtdItem)}</span>
                                <span className="col-span-1.5 min-w-0 break-words text-right font-mono text-stone-500">R$ {formatarPrecisao(totalCpuComBdi)}</span>
                                <span className="col-span-2 text-right font-mono text-stone-900">
                                  R$ {fmt(resumoVenda.total)}
                                </span>
                              </div>
                              ...
                              {isCpuAberta && (item.insumos || []).length > 0 && (
                                <div className="bg-stone-50/50 divide-y divide-stone-100/60 border-t border-b border-stone-100">
                                  {ordenarLista(item.insumos || [], ordenacaoPlanilhaVenda, (insumo, key) => valorVendaInsumo(insumo, qtdItem, key)).map((insumo, idxInsumo) => {
                                    const numInsumo = `${numCpu}.${idxInsumo + 1}`;
                                    const precoVendaInsumo = valorVendaUnitarioInsumo(insumo, bdiCalc, cpus, catalogMap);
                    ...
                    {materiaisOrdenados.map((material) => {
                      ...
                      return (
                        <div key={material.chave} className={`grid grid-cols-12 gap-2 px-4 py-2 text-xs items-center uppercase ${marcado ? "bg-emerald-50/60" : "hover:bg-stone-50/60"}`}>
                          ...
                          <span className="col-span-1.5 text-right font-mono text-stone-900">{formatarPrecisao(material.quantidade)}</span>
...
function ClientesTab({ clientes, projetos, onSalvar, saving, dirty }) {
  ...
  const iniciarNovoCliente = () => {
    setRascunho({ id: "", ...dadosClienteCompartilhado({}) });
    setMensagem("");
  };
  ...
  const salvarCadastro = async () => {
    ...
    if (!nome) {
      setMensagem("Informe o nome ou a razão social do cliente.");
      return;
    }

    const documento = String(rascunho.documento || "").replace(/\D/g, "");
    ...
    const duplicado = (clientes || []).find((cliente) => {
      if (cliente.id === rascunho.id) return false;
      const documentoExistente = String(cliente.documento || "").replace(/\D/g, "");
      if (documento && documentoExistente === documento) return true;
      const telefoneExistente = String(cliente.telefone || "").replace(/\D/g, "");
      return (
        nomeNormalizado === normalizarBusca(cliente.nome || "") &&
        telefone &&
        telefone === telefoneExistente
      );
    });
    if (duplicado) {
      setMensagem(`Já existe um cadastro correspondente a "${duplicado.nome}".`);
      return;
    }

    const clienteAtualizado = {
      id: rascunho.id || uid(),
      ...dadosClienteCompartilhado({ ...rascunho, nome }),
      atualizadoEm: new Date().toISOString(),
    };
    const existe = (clientes || []).some((cliente) => cliente.id === clienteAtualizado.id);
    const proximosClientes = existe
      ? clientes.map((cliente) => (cliente.id === clienteAtualizado.id ? clienteAtualizado : cliente))
      : [...clientes, clienteAtualizado];
    const salvou = await onSalvar(proximosClientes);
    if (salvou) {
      setRascunho(clienteAtualizado);
      setMensagem("Cliente salvo e disponível para todos os orçamentos.");
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.5fr)] gap-4 items-start">
      <div className="bg-white border border-stone-200 shadow-sm rounded-lg overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-base font-semibold text-stone-800 flex items-center gap-2">
                <User size={17} /> Clientes
              </h2>
              <p className="text-xs text-stone-500 mt-1">Cadastro mestre compartilhado entre os operadores.</p>
            </div>
            <button
              type="button"
        ...
        <div className="max-h-[620px] overflow-y-auto divide-y divide-stone-100">
      ...
      <div className="bg-white border border-stone-200 shadow-sm rounded-lg overflow-hidden">
        {!rascunho ? (
          <div className="p-12 text-center text-stone-400">
            <Building2 size={35} className="mx-auto mb-3 opacity-30" />
            <p className="text-sm font-medium">Selecione um cliente ou crie um novo cadastro.</p>
          </div>
        ) : (
          <>
            <div className="px-5 py-4 border-b border-stone-200 flex items-start justify-between gap-4">
              <div>
                <h3 className="text-base font-semibold text-stone-800">
                  {rascunho.id ? "Editar cliente" : "Cadastrar cliente"}
                </h3>
                <p className="text-xs text-stone-500 mt-1">
                  Estes dados serão reutilizados na criação de novos orçamentos.
                </p>
              </div>
              {dirty && <span className="text-[11px] font-semibold text-amber-700">Alterações pendentes</span>}
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <CampoCliente
                  ...
                  value={rascunho.nome || ""}
                  onChange={(valor) => atualizarRascunho("nome", valor)}
                  icon={<User size={14} />}
                  required
                ...
                <CampoCliente
                  ...
                  onChange={(valor) => atualizarRascunho("documento", formatarCpfCnpj(valor))}
                  icon={<FileText size={14} />}
                  inputMode="numeric"
                  maxLength={18}
                />
                <CampoCliente
                  ...
                  onChange={(valor) => atualizarRascunho("contato", valor)}
                ...
                <CampoCliente
                  ...
                  onChange={(valor) => atualizarRascunho("telefone", formatarTelefone(valor))}
                  icon={<Phone size={14} />}
                  inputMode="tel"
                  maxLength={15}
                  autoComplete="tel"
                ...
                <CampoCliente
                  ...
                  onChange={(valor) => atualizarRascunho("email", valor)}
                  icon={<Mail size={14} />}
                  type="email"
                  autoComplete="email"
                />
                <CampoCliente
                  label="CEP"
                  value={rascunho.cep || ""}
                  onChange={(valor) => atualizarRascunho("cep", formatarCep(valor))}
                  icon={<MapPin size={14} />}
                  inputMode="numeric"
                  maxLength={9}
              ...
              <CampoCliente
                ...
                onChange={(valor) => atualizarRascunho("endereco", valor)}
                icon={<MapPin size={14} />}
              ...
              <div className="pt-3 border-t border-stone-200 flex justify-end">
                <button
                  ...
                  disabled={saving || !String(rascunho.nome || "").trim()}
                  className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-stone-900 text-white rounded-lg hover:bg-stone-800 disabled:opacity-40 disabled:cursor-not-allowed"
                >
...
function CadastroCliente({ projeto, cliente, clientes, setProjetos, setCliente, completo, onContinuar, onGerenciarClientes }) {
  ...
  return (
    <div className="bg-white border border-stone-200 shadow-sm rounded-lg overflow-hidden">
      <div className="px-5 py-4 border-b border-stone-200 flex items-start justify-between gap-4">
        ...
        >
          {completo ? "Cadastro completo" : "Cliente e local obrigatórios"}
        </span>
      </div>
      ...
      <div className="p-5 space-y-5">
        <div className="rounded-lg border border-stone-200 bg-stone-50 p-4">
          <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_auto] gap-3 items-end">
            ...
            <button
              type="button"
              onClick={onGerenciarClientes}
              className="px-3 py-2 text-xs font-medium border border-stone-300 rounded-lg bg-white text-stone-700 hover:bg-stone-100"
            >
              Gerenciar clientes ({clientesOrdenados.length})
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-stone-900 text-stone-100 rounded-lg p-5 flex flex-col justify-between h-full min-h-[320px]">
          <div>
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between"><span className="text-stone-400">Custo Direto Contratado:</span><span className="font-mono">R$ {fmt(grandTotal)}</span></div>
              {materialPorContaCliente(bdiCalc.cliente) && <p className="text-xs text-stone-400">Não inclui os materiais fornecidos pelo cliente. A Planilha de Custo mantém o custo integral da composição.</p>}
...
function CronogramaSemanal({
  ...
  const totaisLiquidosEtapas = useMemo(() => {
    const grupos = montarItensProposta(etapas, bdiCalc, cpus, catalogMap, cliente);
    return ratearMoeda(bdiCalc.valorVenda, grupos.map((grupo) => grupo.total));
  }, [etapas, bdiCalc, cpus, catalogMap, cliente]);

  const linhas = useMemo(
    () =>
      (etapas || []).map((etapa, indice) => {
        const id = etapa.id || `etapa-${indice}`;
        const totalVenda = totaisLiquidosEtapas[indice];
    ...
    [etapas, cronograma.etapas, semanas, totaisLiquidosEtapas]
  );

  const totalProjeto = useMemo(
    () => somarMoeda(linhas.map((linha) => linha.totalVenda)),
    [linhas]
  );
    ...
    () =>
      linhas.map((linha) => {
        ...
        return {
          ...
          somaPercentuais,
          pesoFisico,
          valoresSemanais: ratearMoeda(linha.totalVenda * somaPercentuais / 100, percentuais),
  ...
  const totaisSemanais = useMemo(
    () =>
      Array.from({ length: semanas }, (_, indice) =>
        somarMoeda(linhasCalculadas.map((linha) => linha.valoresSemanais[indice]))
      ),
  ...
  const acumuladoFinanceiro = useMemo(() => {
    ...
    return totaisSemanais.map((valor) => {
      acumulado = somarMoeda([acumulado, valor]);
      return acumulado;
    });
  }, [totaisSemanais]);
  ...
  const totalPlanejado = somarMoeda(totaisSemanais);
    ...
    <div className="bg-white border border-stone-200 shadow-sm rounded-lg overflow-hidden">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 border-b border-stone-200 bg-stone-50">
        <div className="px-5 py-3 border-r border-b lg:border-b-0 border-stone-200">
          ...
          {bdiCalc.descontoNegociacao > 0 && <span className="text-xs text-stone-500">Desconto negociado rateado entre as etapas; total igual à proposta.</span>}
        </div>
